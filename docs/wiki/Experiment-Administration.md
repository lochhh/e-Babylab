...
*Experiment Admin*
![Experiment Admin](https://github.com/user-attachments/assets/9b130c55-4934-4cb7-a2e8-2c9fbaa341f1)
...
3. (Optional) EyeTrackingData: contains the trial details (i.e., trial number, trial label, trial code, grid layout (nrows, ncols)), time t, gaze location (x,y), gaze area (row,col)
4. (Optional) EyeTrackingValidation: contains the last 50 gaze locations (x,y) for each _calibration_ trial, trial details (i.e., trial number, trial label, trial code, target location (target_x,target_y)), accuracy, precision root mean square, precision standard deviation (x,y). See also [validation measures](#validation-measures) for the definitions.
